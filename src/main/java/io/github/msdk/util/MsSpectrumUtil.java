...
public class MsSpectrumUtil {
   ...
   */
  public static @Nonnull Float getTIC(@Nonnull double[] intensityValues, @Nonnull Integer size) {
  ...
  public static @Nullable Integer getBasePeakIndex(@Nonnull double[] mzValues,
    ...
    Integer topIndex = null;
    for (int i = 0; i < size; i++) {
      if ((topIndex == null || intensityValues[i] > intensityValues[topIndex]) && mzRange.contains(
          mzValues[i])) {
        topIndex = i;
      }
    }
  ...
  public static @Nonnull String msSpectrumToString(@Nonnull double[] mzValues,
      @Nonnull double[] intensityValues, @Nonnull Integer size) {
  ...
  public static @Nonnull MsSpectrum filterMsSpectrum(@Nonnull MsSpectrum ms,
    ...
    TreeMap<Double, Double> intesityMzSorted = new TreeMap<>(Comparator.reverseOrder());
    double[] mz = ms.getMzValues();
    double[] intensity = ms.getIntensityValues();
    if (mz.length <= pairsLimit) {
      return ms;
    }

    double[] newMz = new double[pairsLimit];
    double[] newIntensity = new double[pairsLimit];
    ...
    for (int i = 0; i < pairsLimit; i++) {
      Entry<Double, Double> pair = intesityMzSorted.firstEntry();
  ...
  public static List<MsSpectrum> filterMsSpectra(List<MsSpectrum> spectra, int listLimit) {
    ...
    TreeMap<Double, MsSpectrum> orderedSpectra = new TreeMap<>(Comparator.reverseOrder());

    /* Order by largest intensity value */
    for (MsSpectrum ms : spectra) {
      double biggest = 0;
      for (double temp : ms.getIntensityValues()) {
        if (temp > biggest) {
          biggest = temp;
        }
      }
    ...
    for (int i = 0; i < listLimit; i++) {
      Entry<Double, MsSpectrum> pair = orderedSpectra.firstEntry();
      if (pair == null) {
        break;
      }
