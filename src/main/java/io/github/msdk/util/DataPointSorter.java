...
public class DataPointSorter {
   ...
   */
  public static void sortDataPoints(final double[] mzBuffer, final double[] intensityBuffer,
  ...
  private static class DataPointComparator implements Comparator<Integer> {
    ...
    private final double[] intensityBuffer;
    private final SortingProperty prop;
    private final SortingDirection dir;

    DataPointComparator(final double[] mzBuffer, final double[] intensityBuffer,
      ...
      switch (prop) {
        case INTENSITY:
          if (dir == SortingDirection.ASCENDING) {
            return Double.compare(intensityBuffer[i1], intensityBuffer[i2]);
          } else {
            return Double.compare(intensityBuffer[i2], intensityBuffer[i1]);
          }
        case MZ:
          if (dir == SortingDirection.ASCENDING) {
